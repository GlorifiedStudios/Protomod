    ...
    public sealed class RoslynCSharp : ScriptableObject
    {
        ...
        public static void SaveAsset(RoslynCSharp settings)
        {
            ...
            if (AssetDatabase.Contains(settings) == true)
            {
                ...
                //AssetDatabase.SaveAssets();           // This forces all assets to save on large projects which can take a long time. SetDirty should be enough but changes wont be saved if editor crashes.
            }
            else
            {
